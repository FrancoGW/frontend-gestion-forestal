...
// Sin filtro de fecha en getAll: la vista de subgerente y el resto deben ver todos los avances
// (incl. registros recientes p.ej. 27/02/2026). El filtro por período se hace en cada vista.
const filtrarAvancesPorFecha = (avances: any[]) => {
  ...
  return avances.filter((avance) => {
    ...
    return !Number.isNaN(fechaAvance.getTime())
  })
}

export const avancesTrabajoAPI = {
  // Obtener todos los avances de trabajo (sin recorte por fecha; cada vista filtra por período)
  getAll: async (params = {}) => {
    try {
      ...
      if (Array.isArray(data.avances)) return data.avances
